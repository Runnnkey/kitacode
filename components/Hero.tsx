import { ArrowRight, MessageSquare, ShieldCheck, Star, TrendingUp, Zap } from "lucide-react";
        ...
        <section className="relative pt-32 pb-20 overflow-hidden bg-gradient-to-b from-blue-50/60 via-slate-50/50 to-white">
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 flex flex-col items-center text-center">
                ...
                <div className="relative mt-12 w-full max-w-5xl rounded-3xl border border-slate-200/80 bg-white/80 shadow-2xl backdrop-blur-md p-3 sm:p-4">
                    <div className="absolute -top-5 right-4 sm:right-8 z-20 bg-white/95 backdrop-blur-md px-4 py-2 rounded-2xl shadow-lg border border-slate-100 flex items-center gap-3">
                        <div className="w-8 h-8 rounded-xl bg-amber-100 text-amber-600 flex items-center justify-center">
                            <TrendingUp className="w-4 h-4" aria-hidden="true" />
                        </div>
                        <div className="text-left">
                            <span className="font-extrabold text-slate-900 block text-sm leading-none">+240%</span>
                            <span className="text-[10px] sm:text-xs text-slate-500">Pertumbuhan Klien</span>
                        </div>
                    </div>
                    ...
                    {/* Badge Bottom Left */}
                    <div className="absolute -bottom-5 left-4 sm:left-8 z-20 bg-white/95 backdrop-blur-md px-4 py-2 rounded-2xl shadow-lg border border-slate-100 flex items-center gap-3">
                        <div className="w-8 h-8 rounded-xl bg-sky-100 text-sky-600 flex items-center justify-center">
                            <Zap className="w-4 h-4" aria-hidden="true" />
                        </div>
                        <div className="text-left">
                            <div className="flex items-center gap-1.5">
                                <span className="font-extrabold text-slate-900 text-sm">99/100</span>
                                <span className="text-[10px] bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded font-semibold">
                                    Lighthouse
                                </span>
                            </div>
                            <span className="text-[10px] sm:text-xs text-slate-500 block">Core Web Vitals Optimal</span>
                        </div>
                    </div>
                </div>
            </div>
